...
DEFAULT_DEMAND_PATTERNS = [
    ...
    r"没买到", r"没票", r"刷不到", r"二开", r"加场", r"抢票", r"手慢",
]

BROAD_INTENT_PATTERNS = [
    r"想去", r"好想去", r"很想去", r"特别想去", r"太想去", r"想看", r"好想看",
    r"去现场", r"现场.*氛围", r"线下.*氛围", r"带我", r"带我去", r"送我去",
    r"抽我", r"看看我", r"捞捞我", r"翻我", r"选我", r"中我", r"让我去",
    r"许愿.*抢到", r"祝我抢到", r"能不能去", r"想拥有", r"想要.*票",
]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect weibo leads from post detail pages.")
    ...
    parser.add_argument("--max-leads", type=int, default=50, help="Target lead count")
    parser.add_argument("--max-pages-per-keyword", type=int, default=10, help="Max search result pages for each keyword")
    parser.add_argument("--max-posts-per-page", type=int, default=8, help="Max high-priority posts to open on each search page")
...
def signal_score(text: str) -> tuple[int, list[str]]:
    ...
    for pat in DEFAULT_DEMAND_PATTERNS:
        if re.search(pat, text or "", flags=re.I):
            hits.append(pat)
    strong_score = len(hits)
    for pat in BROAD_INTENT_PATTERNS:
        if re.search(pat, text or "", flags=re.I):
            hits.append(f"broad:{pat}")
    return strong_score * 2 + (len(hits) - strong_score), hits


def has_strong_signal(hits: list[str]) -> bool:
    return any(not hit.startswith("broad:") for hit in hits)


def intent_label_for(score: int, hits: list[str]) -> str:
    if score >= 4 or len([hit for hit in hits if not hit.startswith("broad:")]) >= 2:
        return "明确求票"
    if has_strong_signal(hits):
        return "疑似求票"
    return "强意愿待确认"


def confidence_for(score: int, hits: list[str]) -> float:
    if has_strong_signal(hits):
        return min(0.55 + score * 0.08, 0.95)
    return min(0.42 + score * 0.06, 0.68)
...
def score_source_context(card: dict) -> int:
    text = normalize_text(" ".join([
        str(card.get("author", "")),
        str(card.get("body_text", "")),
        str(card.get("card_text", "")),
    ]))
    score = 0
    if re.search(r"官方|工作室|后援会|超话|主办|巡演|演唱会", text, flags=re.I):
        score += 2
    if re.search(r"大麦|猫眼|票星球|票务|开票|预售|回流|门票", text, flags=re.I):
        score += 2
    return score
...
def build_comment_lead(event_query: str, keyword: str, page_no: int, card: dict, comment: dict, score: int, hits: list[str]) -> dict:
    ...
    return {
        ...
        "intent_label": intent_label_for(score, hits),
        "is_demand": True,
        "confidence": confidence_for(score, hits),
        "evidence": hits,
        "recommended_action": recommended_action,
        "outreach_angle": "从评论中的求票/没抢到/蹲票等表达切入，确认场次、张数与预算",
        "needs_agent_review": not has_strong_signal(hits),
        "review_reason": "" if has_strong_signal(hits) else "评论表达出强观看意愿，但没有明确票务词，建议二次确认",
        "captured_at": now_iso(),
    }
...
def build_lead(event_query: str, keyword: str, page_no: int, card: dict, score: int, hits: list[str]) -> dict:
    ...
    return {
        ...
        "needs_agent_review": not has_strong_signal(hits),
        "review_reason": "" if has_strong_signal(hits) else "内容表达出强观看意愿，但没有明确票务词，建议二次确认",
        "captured_at": now_iso(),
    }
...
def collect_leads(args: argparse.Namespace) -> dict[str, Any]:
    ...
    run_root = Path(args.run_root) if args.run_root else (ROOT / "runs" / "weibo-leads" / run_id)
    ...
    run_payload = {
        ...
        "operator": "src.flows.weibo_lead_collect",
    }
    ...
    summary = {
        ...
        "note": "正式 lead collect 结果优先按搜索结果页的时间/评论数挑选高热帖子，并进入帖子主页深翻评论；task 框架会将本次 run 结果与 task 级 current_leads 进行增量合并。",
    }
