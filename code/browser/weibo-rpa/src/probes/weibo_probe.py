...
def run(task: TaskInput, run_dir: Path) -> PlatformProbeResult:
    ...
    warnings: list[str] = ["当前仅完成 probe 底座，尚未执行真实微博页面探测。"]
    checks = {
    ...
    log_debug("weibo probe prepared", run_dir=str(run_dir), playwright_importable=playwright_importable)

    return PlatformProbeResult(
        platform="weibo",
        ok=True,
        summary="微博 probe 底座已运行，已完成依赖探测与报告落盘。",
        run_dir=str(run_dir),
